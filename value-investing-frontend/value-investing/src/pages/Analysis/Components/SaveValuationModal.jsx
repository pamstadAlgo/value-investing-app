...
const SaveValuationModal = ({ open, onClose, qfsSymbol, equityVals }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle
        sx={{ fontFamily: "var(--font-family)", fontWeight: "bold" }}>
        SAVE ANALYSIS:{" "}
        <span style={{ color: "var(--action-color)" }}>{qfsSymbol}</span>
      </DialogTitle>
      <DialogContent>
            ...
            <FormControl fullWidth variant="standard">
              ...
              <Select
                ...
                <MenuItem value={0}>
                  Bear{" "}
                  {equityVals?.[0]
                    ? `(${(equityVals[0] / nrShares).toFixed(2)})`
                    : ""}
                </MenuItem>
                <MenuItem value={1}>
                  Base{" "}
                  {equityVals?.[1]
                    ? `(${(equityVals[1] / nrShares).toFixed(2)})`
                    : ""}
                </MenuItem>
                <MenuItem value={2}>
                  Bull{" "}
                  {equityVals?.[2]
                    ? `(${(equityVals[2] / nrShares).toFixed(2)})`
                    : ""}
                </MenuItem>
              </Select>
            </FormControl>
