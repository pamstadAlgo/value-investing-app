...
import { fetchValuationHistory, clearHistory } from '../../../features/valuationHistorySlice';
import { 
  Box, Typography, List, ListItem, ListItemButton, Divider, Chip, CircularProgress 
} from '@mui/material';
...
const HistoryViewer = ({ qfsSymbol, onClose }) => {
  ...
  const loadHistoricalData = (valuation) => {
    ...
    if (inputs && inputs.valuationData) {
      if(window.confirm(`Load snapshot from ${new Date(valuation.created_at).toLocaleDateString()}?\n\nThis will overwrite your current work.`)) {
      ...
      {status === 'loading' ? (
        ...
        <List sx={{ flex: 1, overflowY: 'auto' }}>
            {history && history.length > 0 ? (
                history.map((val) => (
                <ListItem key={val.id} disablePadding sx={{ mb: 1, border: '1px solid var(--border-input-fields)', borderRadius: '4px', bgcolor: 'var(--background-glass-card)' }}>
                    <ListItemButton 
                        onClick={() => loadHistoricalData(val)}
                        sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', p: 1.5 }}
                    >
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', mb: 0.5, alignItems: 'center' }}>
                        <Typography variant="subtitle2" sx={{ fontFamily: 'var(--font-family)', fontWeight: 'bold', color: 'var(--action-color)', fontSize: '1rem' }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', mb: 1 }}>
                        <Typography variant="caption" sx={{ color: 'gray', display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            <PersonIcon sx={{ fontSize: 14 }}/> {val.analyst_name || val.username || "Me"}
                        </Typography>
                    ...
                    <Box sx={{ display: 'flex', width: '100%', justifyContent: 'flex-end' }}>
                         <Typography variant="caption" sx={{ color: 'var(--action-color)', display: 'flex', alignItems: 'center', cursor: 'pointer', fontWeight: 600 }}>
