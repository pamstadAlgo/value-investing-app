...
def get_destination_folder(country):
    """Returns the destination folder where the download data should be stored.
    Possible values: canada, australia, unitedkingdom, europe, usa
    """
    if country.lower() not in ["canada", "australia", "unitedkingdom", "europe", "usa"]:
    ...
    # Construct the path to the Data folder
    data_dir = base_dir / "Data" / country

    print('data_dir that we return: ', data_dir)
    print(f'base_dir: {base_dir}, country: {country}')
...
def download_and_extract_zip(url, destination):
    ...
    if response.status_code == 200:
        ...
        print(f"Files successfully downloaded and extracted to {destination}")
